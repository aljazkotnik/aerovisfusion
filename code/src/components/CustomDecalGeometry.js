...
export default class CustomDecalGeometry extends BufferGeometry {

	constructor( mesh, position, orientation, size ) {

		super();
		
		// Now it does work correctly, just need to figure out hte angles.
		var triangleNormal = new Vector3(0,0,0);
		
		var viewDirection = new Vector3(0,0,1);
		viewDirection.applyEuler(orientation);
		viewDirection.transformDirection( mesh.matrixWorld );
		...
		function generate() {
			...
			for(let i=0; i<decalVertices.length; i+=3){
				...
				triangleNormal.add( decalVertices[i+0].normal );
				triangleNormal.add( decalVertices[i+1].normal );
				triangleNormal.add( decalVertices[i+2].normal );
				
				if( triangleNormal.dot(viewDirection) >= -0.4 ){
					decalVerticesOneSided.push( decalVertices[i+0] )
					decalVerticesOneSided.push( decalVertices[i+1] )
					decalVerticesOneSided.push( decalVertices[i+2] )
				}
			} // for
