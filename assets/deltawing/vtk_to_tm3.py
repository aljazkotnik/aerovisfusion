...
def getDataArrayDataFromFilename(filename):
    tree = ET.parse( filename )
    root = tree.getroot()
    appended_data_string = root.find("./AppendedData").text.strip()[1:]
    return getDataArrayData(root, appended_data_string)



def extractData(data_string, start, end, type_now):
    
    buffer = base64.b64decode(data_string[start:end])
    ...
    if ( type_now == 'Float32' ):
        data = np.frombuffer( buffer, dtype=np.float32, offset=8)
        d = np.float32(data)
        
    if ( type_now == 'Int32' ):
        data = np.frombuffer( buffer, dtype=np.int32, offset=8)
        d = np.int32(data)
    
    return d


def getDataArrayData(root, appended_data_string):
    data_arrays=[]
    for data_array in root.iter('DataArray'):
        ...
        data_array_now = {
            ...
            'type':d['type'], 
            'offset':int(d['offset']),
            'NumberOfComponents':int(d["NumberOfComponents"]) if "NumberOfComponents" in d.keys() else 1
        }
    ...
    for indx, data_array in enumerate(data_arrays):
        ...
        data_array['data'] = extractData(appended_data_string, offset_now, offset_nxt, type_now)
    
    return data_arrays



def interpretPolygonData(polys, data_arrays):
    ...
    return nverts, ntris, vertices, indices
...
def read_vtp_vtu(fname, prop):
    ...
    # Data array tags that contain pointer within the encoded data.
    data_arrays = getDataArrayData(root, appended_data_string)
            
            
    # Look for polygon data
    if ( file_type == "vtp"):
    ...
    nverts, ntris, vertices, indices = interpretPolygonData(polys, data_arrays)
...
def read_pvtp(fname):
    ...
    file_names = []
    for data_set in root.iter('Piece'):
        d = data_set.attrib
        try:
            file_names.append(d['Source'])
        except:
            pass
